...
- **Initial Setup**: Build a graph of your infrastructure for agent context
- **Continuous Updates**: Keep the graph current with the `--interval` option
- **Troubleshooting**: Use logs to debug issues with graph building
- **Environment Management**: Maintain separate graphs for different environments using profiles
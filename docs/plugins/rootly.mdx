...
passing in a Rootly incident webhook payload to `unpage agent run`:

```bash
uv run unpage agent run <agent_name> --payload '{"incident": {"id": "01234567-89ab-cdef-0123-456789abcdef", "attributes": {...}, "type": "incidents"}}'
```

Alternatively, you can pass a simple incident ID and construct the payload manually:

```bash
uv run unpage agent run <agent_name> --payload '{"incident_id": "01234567-89ab-cdef-0123-456789abcdef"}'
```

Unpage will process the incident data and send it to your Agent. The

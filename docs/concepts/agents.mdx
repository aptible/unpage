...
The optional schedule section enables agents to run periodically on a cron schedule. This is useful for "background chores" like cost optimization reports, security audits, and routine maintenance tasks.

Unpage supports multiple cron formats:

```yaml
# Standard 5-field cron (minute precision)
schedule:
  cron: "0 10 2 * *"  # Run at 10:00 AM on the 2nd day of every month

# Extended 6-field cron (second precision)
schedule:
...
schedule:
  ...
  cron: "@weekly"  # Run weekly on Sunday at midnight
  cron: "@monthly" # Run monthly on the 1st at midnight
```
...
See the [schedule command documentation](/commands/agent#subcommand-schedule) for more details on cron formats and running scheduled agents.
...
```bash
# Start the scheduler to run all agents with schedules

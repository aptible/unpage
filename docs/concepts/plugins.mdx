...
1. **Building the Knowledge Graph**: Plugins gather infrastructure data from various sources and add it to the knowledge graph.
2. **Providing LLM Tools**: Plugins expose capabilities to LLMs through the MCP server, enabling agents to interact with your infrastructure.
...
```mermaid
graph LR
    A[AWS Plugin] -->|Discovers| B[EC2 Instance]
    A -->|Discovers| C[RDS Database]
    A -->|Discovers| D[Load Balancer]
    B -->|Connected to| D
    C -->|Connected to| B
...
Most plugins follow these common patterns:

### Graph-Building Plugins

These plugins focus on discovering resources and building the knowledge graph:

```yaml
# Example: AWS plugin configuration
plugins:
  aws:
    enabled: true
...
### Tool-Focused Plugins

Some plugins primarily provide tools without adding significantly to the graph:

```yaml
# Example: Core plugin configuration
plugins:
  core:
    enabled: true  # Provides utility tools like datetime conversion
```

### Hybrid Plugins

Many plugins serve both purposes, adding to the graph and providing related tools:
...
plugins:
  kubernetes:
    enabled: true
    # Builds graph from k8s resources
    # Provides tools to interact with those resources
```

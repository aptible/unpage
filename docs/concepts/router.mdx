...
The Unpage Router is the intelligent system that determines which agent should handle an incoming alert or payload. Rather than requiring you to manually select the right agent for each situation, the Router automatically matches alerts to the most appropriate specialized agent.
...
flowchart TD
    ...
    B --> C[Extract Alert Information]
    C --> D[LLM-Based Matching]
    D --> E{Best Match?}
    E -- CPU Alert Agent --> F[CPU Alert Agent]
    E -- Network Alert Agent --> G[Network Alert Agent]
    E -- Database Alert Agent --> H[Database Alert Agent]
    E -- No Match --> I[Default Agent]
```
...
If no agent's description sufficiently matches the incoming alert, the Router falls back to a default agent. You can identify the default agent by looking for a description like:
...
Under the hood, the Router uses a special "routing agent" configuration that:

1. Has access to all agent descriptions
2. Uses LLM reasoning to evaluate alert characteristics
3. Selects the most appropriate agent based on defined criteria

This approach leverages the LLM's natural language understanding to make intelligent routing decisions without requiring complex rule-based logic.

## Router Configuration

The routing behavior is controlled by system configuration and doesn't require separate configuration. When you run the agent server with:

```bash
unpage agent serve
```

The Router is automatically engaged to direct incoming alerts to appropriate agents.

## Common Routing Scenarios

The Router handles various scenarios:
